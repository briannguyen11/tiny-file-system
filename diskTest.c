...
int main() {
    ...
    int testBlocks[NUM_TEST_BLOCKS] =
        TEST_BLOCKS; /* to contain a number of blocks to test */
    ...
    for (index = 0; index < NUM_TEST_DISKS; index++) {
        ...
        if (disks[index] < 0) {
            ...
            disks[index] = openDisk(
                diskName, BLOCKSIZE * NUM_BLOCKS); /* create the disk */
            if (disks[index] < 0) {
                printf(
                    "] openDisk() failed to create a disk. This should never "
                    "happen. Exiting. \n");
                exit(0);
            }
            ...
            for (index2 = 0; index2 < NUM_TEST_BLOCKS; index2++) {
                ...
                if (retValue < 0) {
                    printf(
                        "] Failed to write to block %i of disk %s. Exiting "
                        "(%i).\n",
            ...
            for (index2 = 0; index2 < NUM_TEST_BLOCKS; index2++) {
                ...
                for (index3 = 0; index3 < BLOCKSIZE; index3++) {
                    if (buffer[index3] != '$') {
                        printf(
                            "] Failed. Byte #%i of block %i of disk %s was "
                            "supposed to be a \"$\". Exiting\n.",
            ...
            printf(
                "] Previous writes were varified. Now, delete the .dsk files "
                "if you want to run this test again.\n");
        }
    }
    return 0;
}
